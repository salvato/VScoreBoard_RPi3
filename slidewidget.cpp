...
SlideWidget::prepareNextSlide() {
    ...
    if(slideList.isEmpty())
        newImage.load(":/Logo_SSD_UniMe.png");
    else
...
SlideWidget::initShaders() {
    QStringList fShaderList = QStringList({"fBookFlip",
                                           ...
                                           "fWind",
                                           "fCrosswarp"
                                           //"fFilmBurn",// Too Slow on Raspberry !!!
                                           //"fDoomScreen",// Non funziona bene !!!
                                           //"fPolkaDotsCurtain",// Non funziona Bene
                                           //"fSwap",// Non funziona RPi3
                                          });
    for(int i=0; i<fShaderList.count(); i++) {
        QOpenGLShaderProgram* pNewProgram = new QOpenGLShaderProgram(this);
        if (!pNewProgram->addShaderFromSourceFile(QOpenGLShader::Vertex, ":/ShadersRPi3/vShader.glsl")) {
            exit(EXIT_FAILURE);
            return;
        }
        QString sFshader = QString(":/ShadersRPi3/%1.glsl").arg(fShaderList.at(i));
